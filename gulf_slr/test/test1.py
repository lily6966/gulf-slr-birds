...
def analysis(species, indiv_prob, input_label, printNow=False):
    ...
    if printNow:
        ...
        print(
            f"TP={TP/total:.6f}, TN={TN/total:.6f}, FN={FN/total:.6f}, FP={FP/total:.6f}"
        )
...
def test_step(classifier, data, test_idx):
    ...
    for i in range(N_test_batch):
        ...
        # Compute loss
        nll_loss, l2_loss, total_loss = classifier.compute_losses(
            input_nlcd, input_label
        )
    ...
    print(
        f"Performance on test set: nll_loss={nll_loss:.6f}, l2_loss={l2_loss:.6f}, total_loss={total_loss:.6f}\n{time_str}"
    )
...
def main(_):
    ...
    test_idx = (
        np.load(FLAGS.test_idx)
        if "esrd" not in FLAGS.data_dir
    ...
    np.save(
        os.path.join(FLAGS.visual_dir, f"feature_emb_{FLAGS.mon}"), feature_embedding
    )
