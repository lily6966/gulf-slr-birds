...
class MODEL(tf.keras.Model):
    def __init__(self, is_training):
        ...
        self.input_nlcd = tf.keras.Input(
            shape=(None, FLAGS.nlcd_dim), dtype=tf.float32, name="input_nlcd"
        )
        self.input_label = tf.keras.Input(
            shape=(None, FLAGS.r_dim), dtype=tf.float32, name="input_label"
        )
        self.keep_prob = tf.keras.Input(
            shape=(), dtype=tf.float32, name="keep_prob"
        )  # for dropout (if used)
        ...
        self.r_mu = tf.keras.layers.Dense(
            units=self.r_dim,
            activation=None,
        ...
        self.r_sqrt_sigma = tf.Variable(
            np.random.uniform(
                -np.sqrt(6.0 / (self.r_dim + FLAGS.z_dim)),
                np.sqrt(6.0 / (self.r_dim + FLAGS.z_dim)),
                (self.r_dim, FLAGS.z_dim),
            ),
            dtype=tf.float32,
            name="r_sqrt_sigma",
    ...
    def call(self, inputs, is_training=False):
        ...
        norm = 0.5 * (1 + tf.math.erf(sample_r / tf.sqrt(2.0)))
        E = norm * (1 - self.eps1) + self.eps1 * 0.5
        ...
        sample_nll = -(
            tf.math.log(E) * input_label + tf.math.log(1 - E) * (1 - input_label)
        )
        ...
        cross_entropy = tf.math.log(
            tf.clip_by_value(indiv_prob, 1e-8, 1.0)
        ) * input_label + tf.math.log(tf.clip_by_value(1.0 - indiv_prob, 1e-8, 1.0)) * (
            1 - input_label
        )
