...
def train_step(hg, input_nlcd, input_label, optimizer, step, writer):
    ...
    indiv_prob, nll_loss, marginal_loss, l2_loss, total_loss = (
        indiv_prob.numpy(),
        nll_loss.numpy(),
        marginal_loss.numpy(),
        l2_loss.numpy(),
...
def validation_step(hg, data, valid_idx, writer, step):
    ...
    for i in range(
        0, len(valid_idx) - (len(valid_idx) % FLAGS.batch_size), FLAGS.batch_size
    ):
        ...
        indiv_prob, nll_loss, marginal_loss, l2_loss, total_loss = hg(
            [input_nlcd, input_label], is_training=False
        )
...
def main(_):
    ...
    learning_rate_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
        initial_learning_rate=0.1, decay_steps=10000, decay_rate=0.96, staircase=True
    )
    ...
    for epoch in range(FLAGS.max_epoch):
        ...
        for i in range(one_epoch_iter):
            ...
            indiv_prob, nll_loss, marginal_loss, l2_loss, total_loss = train_step(
                hg, input_nlcd, input_label, optimizer, current_step, summary_writer
            )
            ...
            if (i + 1) % FLAGS.check_freq == 0:
                ...
                with summary_writer.as_default():
                    ...
                    tf.summary.scalar(
                        "train/nll_loss", mean_nll_loss, step=current_step
                    )
                    tf.summary.scalar(
                        "train/marginal_loss", mean_marginal_loss, step=current_step
                    )
                    tf.summary.scalar("train/l2_loss", mean_l2_loss, step=current_step)
                    tf.summary.scalar(
                        "train/total_loss", mean_total_loss, step=current_step
                    )

                time_str = datetime.datetime.now().isoformat()

                print(
                    "train step: %s\tap=%.6f\tnll_loss=%.6f\tmarginal_loss=%.6f\tl2_loss=%.6f\ttotal_loss=%.6f"
                    % (time_str, ap, nll_loss, marginal_loss, l2_loss, total_loss)
                )
