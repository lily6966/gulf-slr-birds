...
for i in range(1, 400):
    for j in range(1, 13):
        command = "python3 train.py --model_dir ./data/small_model_mon%d/model_%d_%d --summary_dir ./summary_%d_%d --visual_dir ./vis_%d_%d --data_dir ./data/small_region/small_case%d_bird_%d.npy --train_idx ./data/small_region/small_case%d_bird_train_idx_%d.npy --valid_idx ./data/small_region/small_case%d_bird_val_idx_%d.npy --test_idx ./data/small_region/small_case%d_bird_test_idx_%d.npy --r_dim 404 --r_max_dim 404 | tee ./data/small_map_data_%d/ebird_%d_%d" % (j, i, j, i, j, i, j, i, j, i, j, i, j, i, j, j, i, j)
        print (command)
        os.system(command)
